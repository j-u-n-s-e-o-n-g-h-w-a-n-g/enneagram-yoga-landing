...
async function initDB() {
  ...
  try {
    ...
    // updated_at columns for change tracking
    await client.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW();`);
    await client.query(`ALTER TABLE class_passes ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW();`);
    await client.query(`ALTER TABLE payments ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW();`);
    await client.query(`ALTER TABLE applications ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW();`);
    ...
    await client.query(`
      ...
      $$ LANGUAGE plpgsql;
    `);
    for (const tbl of ['users', 'class_passes', 'payments', 'applications']) {
      await client.query(`
        DO $$ BEGIN
          IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trg_${tbl}_updated_at') THEN
            CREATE TRIGGER trg_${tbl}_updated_at BEFORE UPDATE ON ${tbl} FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
          END IF;
        END $$;
      `);
    }
