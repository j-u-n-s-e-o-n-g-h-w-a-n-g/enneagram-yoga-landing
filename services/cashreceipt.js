  ...
  async function issueCashReceipt({ phone, name, email, amount, memo }) {
    ...
    try {
      ...
      if (!tokenData.session_token) {
        console.error('Popbill token error:', JSON.stringify(tokenData));
        return { success: false, error: 'Popbill 토큰 발급 실패' };
      }
      ...
      const issueTimeoutId = setTimeout(() => issueController.abort(), 10000);
      try {
        ...
        clearTimeout(issueTimeoutId);
        console.error('Cashbill issue error:', err.message);
        return { success: false, error: err.message };
      }
    } catch (err) {
      clearTimeout(tokenTimeoutId);
      console.error('Popbill token fetch error:', err.message);
      return { success: false, error: err.message };
    }
  }
