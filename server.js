...
app.post('/api/setup/promote-admin', requireDB, async (req, res) => {
  const { email, secret } = req.body;
  if (secret !== 'enneagram-yoga-admin-setup-2026') {
    return res.status(403).json({ error: 'Invalid secret' });
  }
  const pool = getPool();
  try {
    const result = await pool.query("UPDATE users SET role = 'admin' WHERE email = $1 RETURNING id, name, email, role", [email]);
    if (result.rows.length === 0) return res.status(404).json({ error: '해당 이메일의 사용자를 찾을 수 없습니다' });
